...
function bodyToEntity(body, existingCreatedAt) {
  ...
  const entity = {
    ...
    cuisine: safe(body.cuisine),
    area: safe(body.area),
    mustTry: safe(body.mustTry),
    bestTime: safe(body.bestTime),
    outingPlaceId: safe(body.outingPlaceId),
    restaurantId: safe(body.restaurantId),
    mealSlot: safe(body.mealSlot),
